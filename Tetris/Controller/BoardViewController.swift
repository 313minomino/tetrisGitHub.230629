...
class BoardViewController: UIViewController{
    ...
    private func defineBoard(){
        let startingSquareSize: Float = 50
    ...
    private func drawBoardGrid(){
        ...
        gridView.frame = CGRect(x: 0, y: 0, width: playableBoardView.bounds.width, height: playableBoardView.bounds.height)
    ...
    private func drawSquare(_ square: Square, _ color: UIColor){
        let squareView = TetrominoDrawer.generateSquareView(
            ...
            squareSize: squareSize - 2, // マスのサイズを調整する値
            color: color)
